...
public class HummingBirdAgent : Agent {
    ...
    // Maximum distance from the beak tips to accept nectar collision
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        //Calculat new pitch and yaw based on smoothed values
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Get a vector from the beak tip to the nearesr flower
        Vector3 toFlower = nearestFlower.FlowerCenterPosition - beakTip.position;

        // Observe a normalized vector pointing to the nearest flower (3 observation)
        sensor.AddObservation(toFlower.normalized);

        //Observe a dot product that indicates whether the beak tip is front of the flower
        // (+1 means that the beak tip is directly in front of the flower, -1 means directly behind)
        sensor.AddObservation(Vector3.Dot(toFlower.normalized, -nearestFlower.FlowerUpVector.normalized));

        // Observe a dot product that indicates whether the beak is pointing towards the flower
    ...
    /// this function will be ccalled. It;s return value will be fed into
    ...
    public override void Heuristic(float[] actionsOut)
    {
        ...
        // Add 3 movement values, pitch & yaw to the actionsOut array
    ...
    /// <summary>
    /// Move the agent to a safe random position (i.e. does not collide with any gameObjects)
    ...
    private void UpdateNearestFlower()
    {
        foreach (Flower flower in flowerArea.Flowers)
        {
            ...
            else if (flower.HasNector)
            {
                ...
                // If current nearesr flower is empty OR this flower is closer, update the nearest flower
    ...
    {
        //Check if agent is collider with nectar
        if (collider.CompareTag("nectar"))
        {
            Vector3 closestPointToBeakTip = collider.ClosestPoint(beakTip.position);

            // Check if the closest collision point is close to beak tip
            // Note: collision with anything but the beak tip should not count
    ...
    {
        // Avoids scenario where nearest flower nectar is stolen by oppenent
